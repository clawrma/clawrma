...
async function writeStreamingInferenceResponse(
  ...
  const pendingToolCallLines: string[] = [];
  for await (const line of iterateResponseLines(response.body)) {
    ...
    for (const event of events) {
      if (event.type === "text") {
        ...
        if (pendingToolCallLines.length > 0) {
          for (const toolCallLine of pendingToolCallLines) {
            stdout.write(toolCallLine);
            stdout.write("\n");
          }
          pendingToolCallLines.length = 0;
        }
      ...
      const renderedToolCall = formatTaggedInferenceLine(
        "tool_call_delta",
        event.toolCall,
      );
      if (!sawTextOutput) {
        pendingToolCallLines.push(renderedToolCall);
        continue;
      }
  ...
  if (!sawTextOutput && pendingToolCallLines.length > 0) {
    stdout.write(INFERENCE_WARNING_TEXT);
    stdout.write("\n");
    for (const toolCallLine of pendingToolCallLines) {
      stdout.write(toolCallLine);
      stdout.write("\n");
...
function extractInferenceMessageOutput(value: unknown): string {
  ...
  if (!sawVisibleContent && sawToolCalls) {
    renderedLines.unshift(INFERENCE_WARNING_TEXT);
  }
